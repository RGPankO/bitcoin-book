#!/usr/bin/env bash
# Download auto-generated BG subtitles from YouTube playlist
#
...
SOURCES_DIR="TASKS/SHADOW-PLAMEN/sources"
IDS_FILE="/tmp/shadow-playlist-ids.txt"
...
# Global backoff state — escalates on consecutive 429s across ANY video
...
while IFS= read -r vid; do
  ...
  # Skip if previously confirmed no subs
  grep -qxF "$vid" "$NO_SUBS_LOG" 2>/dev/null && { echo $((i+1)) > "$PROGRESS"; continue; }

  # Retry loop — only 429 causes retry, and it's a global cooldown
  while true; do
    ...
    if echo "$result" | grep -q "429\|Too Many\|HTTP Error 429"; then
      # THROTTLED — global cooldown, do NOT skip this video
      ...
      sleep $wait
      continue  # retry SAME video after cooldown

    elif echo "$result" | grep -q "Writing video subtitles"; then
      # SUCCESS
      fetched=$((fetched + 1))
      global_backoff_level=0  # reset on success
      ...
      # GENUINELY NO SUBS — skip permanently
      no_subs=$((no_subs + 1))
